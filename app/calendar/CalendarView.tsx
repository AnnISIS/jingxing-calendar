...
export function CalendarView({month,day,onChange,onViewDay,embedded=false}:{month:number;day:number;onChange:(m:number,d:number)=>void;onViewDay:()=>void;embedded?:boolean}){
  ...
  return <main className={`site-shell calendar-site ${embedded?"embedded-view":""}`}>
    ...
    <section className="calendar-sheet">
      <div className={`month-dharma-status ${monthDharma.length?"has-dharma":""}`}><i/><div><strong>{monthDharma.length?`本月有 ${monthDharma.length} 项已核验东林法务`:"本月暂无已发布的东林法务"}</strong><span>{monthDharma.length?"佛七、闭关持续日期均以朱红线标示":"网站不会用往年惯例代替正式公告"}</span></div><button onClick={()=>monthDharma.length?onChange(month,firstDharmaDayInMonth(monthDharma[0],month)):onChange(1,1)}>{monthDharma.length?"定位日期":"查看一月"}</button></div>
      ...
      <div className="calendar-grid">{cells.map((c,i)=>{const e=!c.out?findFestival(month,c.day):undefined,d=!c.out?dharmaOnDate(month,c.day):[];return <button key={i} className={`${c.out?"outside":""} ${!c.out&&day===c.day?"selected":""} ${e?"event-cell sacred":""} ${d.length?"dharma-cell":""}`} disabled={c.out} onClick={()=>!c.out&&onChange(month,c.day)}><strong>{c.day}</strong><small>{month===8&&!c.out?augustLunar[c.day-1]:e?.lunar||""}</small>{e&&<em>{e.short}</em>}{d.length>0&&<i className="dharma-dot" aria-label="东林法务"/>}</button>})}</div>
