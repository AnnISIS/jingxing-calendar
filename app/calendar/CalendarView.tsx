...
export function CalendarView({month,day,onChange,onViewDay,embedded=false}:{month:number;day:number;onChange:(m:number,d:number)=>void;onViewDay:()=>void;embedded?:boolean}){
  ...
  return <main className={`site-shell calendar-site ${embedded?"embedded-view":""}`}>
    ...
    <section className="calendar-sheet">
      ...
      <section className="donglin-dharma"><header><i/>东林法务 <span>官方公开资料</span></header><p>目前收录已确认的2026年1月净行月历及跨年冬季安排，不推测尚未发布的月份。</p>{dharmaEvents.map((e,i)=><a href={e.source} target="_blank" rel="noreferrer" key={i}><time>{formatDharmaDate(e)}</time><span>{e.title}<small>{e.place}</small></span><em>已核验</em></a>)}</section>
