  ...
  return <main className={`site-shell calendar-site ${embedded?"embedded-view":""}`}>
    ...
    <section className="calendar-sheet">
      ...
      <section className="calendar-archives">
        <details><summary><span><i className="dharma"/>东林全年法务</span><b>{dharmaEvents.length} 项</b><em>查看详情</em></summary><div className="archive-body donglin-dharma"><p>已收录2026年东林法务资料；官网历史详情页可能下架，具体安排请以寺院最新公告为准。<a className="dharma-overview" href="https://www.lsdls.cn/#/website/fawuanpai/list" target="_blank" rel="noreferrer">东林法务总览 ›</a></p>{dharmaEvents.map((e,i)=><div className="dharma-record" key={i}><time>{formatDharmaDate(e)}</time><span>{e.title}<small>{e.place}</small></span></div>)}</div></details>
