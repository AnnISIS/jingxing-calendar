...
export function CalendarView({month,day,onChange,onViewDay,embedded=false}:{month:number;day:number;onChange:(m:number,d:number)=>void;onViewDay:()=>void;embedded?:boolean}){
  ...
  const chooseArchiveDate=(event:MouseEvent<HTMLButtonElement>,eventMonth:number,eventDay:number)=>{
    onChange(eventMonth,eventDay);
    event.currentTarget.closest("details")?.removeAttribute("open");
    requestAnimationFrame(()=>window.scrollTo({top:0,left:0,behavior:"smooth"}));
  };
  return <main className={`site-shell calendar-site ${embedded?"embedded-view":""}`}>
    <header className="calendar-hero"><img className="calendar-landscape" src="/summer-mountain.jpg" alt="淡墨夏季山居图"/><div className="calendar-hero-veil"/><div className="calendar-year"><span>丙午年</span><p>二〇二六</p></div><div className="month-switch"><button onClick={()=>move(-1)} aria-label="上个月" disabled={month===1}>‹</button><div><strong>{cn[month-1]}</strong><span>二〇二六年 · 圣日与东林法务</span></div><button onClick={()=>move(1)} aria-label="下个月" disabled={month===12}>›</button></div></header>
    <section className="calendar-sheet">
      ...
      <section className="calendar-archives">
        <details><summary><span><i/>全年佛菩萨纪念日</span><b>{festivals.length} 日</b><em>查看详情</em></summary><div className="archive-body year-festivals">{Array.from({length:12},(_,i)=>i+1).map(m=><div className="festival-month" key={m}><b>{String(m).padStart(2,"0")}</b><div>{festivals.filter(x=>x.month===m).map(e=><button key={e.day} onClick={event=>chooseArchiveDate(event,e.month,e.day)}><time>{e.day}日</time><span>{e.title}<small>农历{e.lunar} · {festivalLevel(e)}</small></span><em>›</em></button>)}</div></div>)}</div></details>
